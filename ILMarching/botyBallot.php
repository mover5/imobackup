...
function validateBallot(div1, div2, div3, div4)
{
	...
	for (i=1; i <= div1; i++) {
			...
			for (j = i+1; j <= div1; j++) {
					otherfieldname = "1_" + j;
					if (document.getElementById(fieldname).selectedIndex != 0 && document.getElementById(fieldname).value == document.getElementById(otherfieldname).value) {
...
if (strtotime($startDate) <= strtotime("now") && strtotime($endDate)+86400 >= strtotime("now")) {
	if (isset($_POST['login'])) {
		$query = "SELECT Password, PASSWORD('".$_POST['password']."') AS hpass, Voted FROM BandsOfTheYearSettings JOIN BandsOfTheYearVoters WHERE Email = '".$_POST['username']."'";
		$result = mysql_query($query, $connect);
		$row = mysql_fetch_array($result);
		if ($row['Voted'] == 1) {
			echo "<center><h2>You have already voted in this years Bands of the Year. Thanks!</h2></center>";
		} else if (mysql_num_rows($result) == 1 &&  $row['Password'] == $row['hpass'] && $row['Voted'] == 0) {
			$divoptions[1] = "";
			$divoptions[2] = "";
			$divoptions[3] = "";
			$divoptions[4] = "";
			...
			echo "<form action='botySubmit.php' method='POST' name='ballot'>";
			...
			echo "<center><h2>Incorrect Username or Password</h2><a href='botyBallot.php'>Try Again</a></center>";
		}
	} else {
		echo "<center><form action='botyBallot.php' method='POST'>";
		echo "<h2>BOTY Login</h2>";
