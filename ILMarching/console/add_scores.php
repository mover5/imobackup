...
if (isset($_POST['modifysubmit'])) {
	...
	$festid = $_POST['FestivalID'];
		...
		$query = "SELECT Name, FestivalYear, GrandChampionID, GrandChampionBandName, ScoreNotes FROM Festivals WHERE FestivalID = $festid";
	...
	$FestivalYear = $row['FestivalYear'];
		...
		if ($num == 0 && $rain == 0) {
			...
			mysql_query($query, $connect);
			$message .= "Show marked as 'Rained Out'";
		} else {
...
$().ready(function() {
	$("#FestivalName").autocomplete("ajax/get_festival_list_for_scores.php", {
		width: 260,
		matchContains: true,
		//mustMatch: true,
		//minChars: 0,
	...
	<div id="contentwrapper">
		<div id="contentcolumn">
			<div class="innertube">
				...
				<h1>Add Scores</h1>
				<?php if (!$displayform) { ?>
				<form name='SelectFestival' action='add_scores.php' method='POST' autocomplete="off">
				...
				<?php } else { 
					...
					echo "<form action='add_scores.php' method='POST'  enctype='multipart/form-data'>";
					echo "<input type='checkbox' name='rain' id='rain' /> <font size='+1'><strong>Rained Out.</strong></font><br />";
					$query = "SELECT BandID, BandName, Schedules.Class, Type FROM Schedules LEFT JOIN ClassOrder ON Schedules.Class = ClassOrder.Class WHERE FestivalID = $festid AND Schedules.Class <> 'Awards' AND Schedules.Class <> 'Break' AND Schedules.Class <> 'Exb' ORDER BY Type DESC, OrderNum ASC, PerformanceTime ASC";
					$result = mysql_query($query, $connect);
					...
					while ($doone == 0 || $row = mysql_fetch_array($result)) {
						$doone = 1;
						$num_row++;
						$score = "";
						$bandid = $row['BandID'];
						...
						<td valign='top'>
							<table border=0>
								...
								<tr valign='top'>
									...
									?>
									<td>&lt;-OR-&gt;</td>
									<td><input type='text' name='GCbandname' id='GCbandname' size='15' value=''/></td>
								</tr>
							</table>
							<br />
							...
							<br />
							<input type='submit' name='changesubmit' id='changesubmit' value='Add Scores' />
						</td>
						<td valign='top'>
							<table border=1>
								<tr>
