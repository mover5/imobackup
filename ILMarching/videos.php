...
function searchVideoURL(search) {
...
function watchVideo(url, title, band, bandid, bandyear, festname, festid, fullurl) {
	...
	setClickHandler(fullurl);
}

function setClickHandler(url) {
    $("#reportvideo").attr('onClick', "reportVideo('"+url+"')");
    $("#reportvideo").show();
    $("#reportVideoDialog").hide();
    $("#reportreturn").html("");
    $("#reportreason").val("");
...
function reportVideo(url) {
    var updateReportButton = function() {
        $("#reportvideo").hide();
    };
    $("#reportVideoDialog").show();
    var left = $("#reportvideo").position().left + ($("#reportvideo").width()/2) - ($("#reportVideoDialog").width()/2),
        prev = $("#reportVideoDialog").position().left * -1;
    $("#reportVideoDialog").offset({left: prev});
    $("#reportVideoDialog").offset({left: left});
    $("#close").click(function() {
        $("#reportVideoDialog").hide();
    });
    
    $("#reportVideoDialog button").click(function() {
        var reportReason = $("#reportreason").val();
        $("#reportVideoDialog").hide();
        $("#reportreturn").load("ajax/report_video.php", {vidurl: url, reason: reportReason}, updateReportButton);
    });
}
...
<center><p><h2>Illinois Marching Online Videos</h2></p></center>
	<center>
...
if (isset($_GET['video'])) {
    ...
    $query = "SELECT BandShows.Title, Videos.BandYear, Festivals.Name, Festivals.FestivalID, Bands.School, Bands.BandID
     ...
     WHERE VideoURL = '$youtubeurl' AND Videos.Approved = 1";
...
$query = "SELECT * FROM Videos WHERE Approved = 1";
$result = mysql_query($query, $connect);
$num = mysql_num_rows($result);
$rand = rand(0, $num-1);

$query = "SELECT v.VideoURL, s.Title, v.BandYear, f.Name, f.FestivalID, b.School, b.BandID FROM Videos v LEFT JOIN BandShows s ON (v.BandYear = s.Year AND v.BandID = s.BandID) LEFT JOIN Festivals f ON v.FestivalID = f.FestivalID LEFT JOIN Bands b ON s.BandID = b.BandID WHERE v.Approved = 1 LIMIT $rand, 1";
$result = mysql_query($query, $connect);
$row = mysql_fetch_array($result);
...
echo "<center><br /><b id='title' name='title'>$title</b><br /><a id='bandlink' name='bandlink' href='bands_indiv.php?BandID=$bandid&year=$bandyear'>$school</a><br /><div id='festinfo'>$fest</div>
<input type='button' id='reportvideo' class='button_submit' value='Report Video'/><br /><div id='reportreturn'></div>
<div class='dialogBox' id='reportVideoDialog'>
<div class='close' id='close'>X</div><br />
<h3>Why are you reporting this video?</h3>
<input type='text' id='reportreason' name='reportreason' size='40' /><p>
<button class='button_submit'>Submit</button></p>
</div>
</center>";
...
<!-- Drill Down -->
<td width=300>
<center><h3 id='ddname' name='ddname'>SEARCH</h3></center><div id='drilldown' name='drilldown' style='height:460;overflow:scroll'>
...
echo "setClickHandler(\"$fullurl\");";
if (isset($_GET['search'])) {
    echo "searchVideoURL(" . $_GET['search'] . ");";
} else {
    echo "selectVideo('r=ALL&s=0');";
}
echo "</script>";
?>
<!--****End Page Content***************-->
