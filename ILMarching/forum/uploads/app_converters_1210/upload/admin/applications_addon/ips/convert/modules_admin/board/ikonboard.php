	...
	*/
	...
	private function convert_forum_info()
	{
		...
		foreach( $loop as $row )
		{
			...
			$res = ipsRegistry::DB('hb')->buildAndFetchAll( array( 'select' => '*', 'from' => 'forum_subscriptions', 'where' => "FORUM_ID = {$row['FORUM_ID']} AND TOPIC_ID = 0" ) );
			foreach( $res AS $k => $sub )
			{
				$savesub = array(
				...
				$this->lib->convertForumSubscription( $sub['ID'], $savesub );
			}
		}
	...
	private function convert_topics()
	{
		...
		while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
		{
			...
			$res = ipsRegistry::DB('hb')->buildAndFetchAll( array( 'select' => '*', 'from' => 'forum_subscriptions', 'where' => "TOPIC_ID = {$row['TOPIC_ID']}" ) );
			foreach( $res AS $k => $sub )
			{
				if ( $sub['TOPIC_ID'] )
				{
					...
					$this->lib->convertTopicSubscription( $sub['ID'], $savesub );
				}
			}
		}
