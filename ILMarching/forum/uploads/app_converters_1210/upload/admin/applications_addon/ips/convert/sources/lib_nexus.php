 ...
 * Last Update: $Date: 2013-09-03 18:25:32 -0400 (Tue, 03 Sep 2013) $
 * Last Updated By: $Author: rashbrook $
 *
 * @package		IPS Converters
 * @author 		Andrew Millne / Ryan Ashbrook / Alex Hobbs
...
class lib_nexus extends lib_master
{
	...
	public function getInfo()
	{
		return "<strong>Rebuild Content</strong><br />
				...
				<a href='{$this->settings['base_url']}&app=core&&module=tools&section=cache' target='_blank'>Click here</a> and recache all.<br />
				<strong>Customers Usergroup (Optional)</strong><br />
				To move all customers to a special Customers usergroup, go to your AdminCP > Other Apps > Nexus > Store Settings > Packages, edit your subscription packages, and set it to promote users to the desired group on purchases. It will ask you if you want to update existing purchases, select yes.";
	}
		...
		switch ($action)
		{
			...
			case 'nexus_attachments':
				return array(
					...
					'where'		=> array( 'attachments'	=> "attach_rel_module = 'support'" )
				);
			break;
			
			case 'nexus_gateways':
				return array(
					'tables'	=> array( 'nexus_gateways' => 'g_id' )
				);
				break;
					
			case 'nexus_package_groups':
				return array(
					'tables'	=> array( 'nexus_package_groups' => 'pg_id' )
				);
				break;

			case 'nexus_purchases':
				return array(
					'tables'	=> array( 'nexus_purchases' => 'ps_id' )
				);
				break;
					
			case 'nexus_packages':
				return array(
					'tables'	=> array( 'nexus_packages' => 'p_id', 'nexus_packages_products'	=>	'p_id' )
				);
				break;
				
			case 'nexus_invoices':
				return array(
					'tables'	=> array( 'nexus_invoices' => 'i_id' )
				);
				break;

			case 'nexus_support_departments':
				return array(
					'tables'	=> array( 'nexus_support_departments' => 'dpt_id' )
				);
				break;
					
			case 'nexus_support_statuses':
				return array(
					'tables'	=> array( 'nexus_support_statuses' => 'status_id' )
				);
				break;

			case 'nexus_support_requests':
				return array(
					'tables'	=> array( 'nexus_support_requests' => 'r_id' )
				);
				break;

			case 'nexus_support_replies':
				return array(
					'tables'	=> array( 'nexus_support_replies' => 'reply_id' )
				);
				break;
				
			case 'nexus_customers':
				return array(
					'tables'	=> array( 'nexus_customers' => 'member_id' )
				);
			break;
			
			case 'nexus_notes':
				return array(
					'tables'	=> array( 'nexus_notes' => 'note_id' )
				);
			break;
			
			case 'nexus_support_severities':
				return array(
					'tables'	=> array( 'nexus_support_severities' => 'sev_id' )
				);
			break;
			
			case 'nexus_support_staff':
				return array(
					'tables'	=> array( 'nexus_support_staff' => 'staff_id' )
				);
			break;
	...
	public function convertPackageGroup($id, $info, $parentLink = true)
	{
		...
		{
			$parent = $this->getLink( $info['parent'], 'nexus_package_groups', true );
			if ( $parent )
			{
			...
			{
				$info['conv_pg_parent']	= $info['parent'];
			}
		}
	 ...
	 * @return 	boolean		Success or fail
	 **/
	public function convertPackage($id, $info, $discounts = array(), $product = array() )
	{
		if ( !$id )
		{
			$this->logError ( $id, 'No ID specified.' );
		...
		// Add in the product
		if ( $info['p_type'] == 'product' )
		{
			$productSave = array(
				'p_id' 				=> $inserted_id,
				'p_physical' 		=> $product['p_physical'],
				'p_subscription'	=> $product['p_subscription'],
				'p_shipping'		=> $product['p_shipping'],
				'p_weight'			=> $product['p_weight'],
				'p_lkey'			=> $product['p_lkey'],
				'p_lkey_identifier'	=> $product['p_lkey_identifier'],
				'p_lkey_uses'		=> $product['p_lkey_uses']
			);
			
			$this->DB->insert( 'nexus_packages_products', $productSave );
			
			$this->addLink( $inserted_id, $id, 'nexus_packages_products' );
		}
	...
	public function convertInvoice($id, $info, $items, $extra)
	{
		...
		if ( !$info['i_title'] )
		{
			$this->logError ( $id, 'No Invoice Title specified.' );
			return false;
		}
		...
		{
			$item['itemID'] = $this->getLink ( $item['itemID'], 'nexus_packages', TRUE );
			$storeItems[]	= $item;
		}
	...
	public function convertSupportRequest($id, $info)
	{
		...
		$info['r_status']			= $this->getLink ( $info['r_status'], 'nexus_support_statuses', TRUE, $this->useLocalLink );
		...
		$info['r_severity']			= $this->getLink ( $info['r_severity'], 'nexus_support_severities', TRUE, $this->useLocalLink );
