 ...
 */
	...
	class admin_convert_board_eblah extends ipsCommand
	{
	    ...
	    public function doExecute( ipsRegistry $registry )
		{
			...
			{
				$paths = array(	'boards'		=> $this->sourceDirectory . '/Boards/bdindex.db',
								'categories'	=> $this->sourceDirectory . '/Boards/bdscats.db',
								'members'		=> $this->sourceDirectory . '/Members/List.txt',
								'ranks'			=> $this->sourceDirectory . '/Prefs/Ranks2.txt',
						);
				...
				if( !empty( $paths ) )
				{
					$missing = implode( ", ", array_keys( $paths ) );
					$this->lib->sendError("The following eBlah files do not exist in the specified location: {$missing}");
				}
