 ...
 * vBulletin Blog
 * Last Update: $Date: 2013-04-10 10:38:12 -0400 (Wed, 10 Apr 2013) $
 ...
 * 
 * This is the same as vbulletin_legacy in other converters. Yay naming conventions.
 */
	...
	class admin_convert_blog_vbblog extends ipsCommand
	{
	    ...
	    */
		...
		private function convert_blog_blogs()
		{
			...
			$main = array(	'select' 	=> 'bu.*',
							'from' 		=> array('blog_user' => "bu"),
							'order'		=> 'bu.bloguserid ASC',
							'add_join'	=> array(
									array( 	'select' => 'u.username',
											'from'   =>	array( 'user' => 'u' ),
											'where'  => "bu.bloguserid=u.userid",
											'type'   => 'left'
									),
							),
						);
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$save = array(
					...
					'blog_name'			=> ($row['title'] == '') ? "Blog {$row['username']}" : $row['title'],
		...
		private function convert_blog_entries()
		{
			...
			$main = array(	'select' 	=> 'b.*',
							...
							'order'		=> 'b.blogid ASC',
							'add_join'	=> array(
											array( 	'select' => 't.*, t.blogid as blog_id',
		...
		private function convert_blog_comments()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				if ( is_array( $us['blog_text_ids'] ) && in_array($row['blogtextid'], $us['blog_text_ids']))
				{
					continue;
				}
