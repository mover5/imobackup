 ...
 * Last Update: $Date: 2013-09-19 14:38:32 -0400 (Thu, 19 Sep 2013) $
 ...
 */
	...
	class lib_calendar extends lib_master
	{
		...
		public function getInfo()
		{
			...
			<br />
			<a href='{$this->settings['base_url']}&app=core&module=tools&section=cache' target='_blank'>Click here</a> and run the 'Recache All' Tool.";
		}
		...
		public function truncate($action)
		{
			switch ($action)
			{
				case 'cal_calendars':
					return array(
						'tables'	=> array( 'cal_calendars' => 'cal_id' )
					);
					break;
					
				case 'cal_events':
					return array(
						'tables'	=> array( 'cal_events' => 'event_id' )
					);
					break;
				
				case 'cal_event_comments':
					return array(
						'tables'	=> array( 'cal_event_comments' => 'comment_id' )
					);
				break;
				
				case 'cal_event_ratings':
					return array(
						'tables'	=> array( 'cal_event_ratings' => 'rating_id' )
					);
				break;
				
				case 'cal_event_rsvp':
					return array(
						'tables'	=> array( 'cal_event_rsvp' => 'rsvp_id' )
					);
				break;
		...
		public function convertEvent($id, $info, $literal=false)
		{
			...
			if ( ! $info['event_post_key'] )
			{
				$info['event_post_key'] = md5( $id . 'event' );
			}
			...
			if ( $info['event_perms'] AND $info['event_perms'] != "*" )
			{
				foreach ( explode( ",", $info['event_perms'] ) as $group )
				{
					$save[] = $this->getLink( $group, 'groups', true, true );
				}
					
				$info['event_perms'] = implode(',', $save);
			}
