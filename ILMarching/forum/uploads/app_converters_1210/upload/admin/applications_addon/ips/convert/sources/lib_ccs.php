 ...
 */
	class lib_ccs extends lib_master
	{
		...
		{
			return "<a href='{$this->settings['base_url']}&app=ccs&module=articles&section=categories&do=recache' target='_blank'><strong>Click here</strong></a> to rebuild categories.<br />
				<br /><a href='{$this->settings['base_url']}&app=ccs&module=settings&section=settings'><strong>Click here</strong></a> and turn the application back online.<br />
			<br /><strong>Please Note:</strong> If you chose to Empty Local Data when Converting Pages, you <strong>will</strong> need to add a default index.html page or adjust the setting 'Default Home Page' <a href='{$this->settings['base_url']}&app=ccs&module=settings&section=settings'><strong>here</strong></a>";
		}
			...
			switch ($action)
			{
				...
				case 'ccs_attachments':
					return array(
						...
						'where'		=> array( 'attachments'	=> "attach_rel_module = 'ccs'" )
					);
					break;
				case 'ccs_blocks':
					return array(
						'tables'	=> array( 'ccs_blocks' => 'block_id' )
					);
					break;
				case 'ccs_containers':
					return array(
						'tables'	=> array( 'ccs_containers' => 'container_id' )
					);
					break;
				case 'ccs_database_categories':
					return array(
						'tables'	=> array( 'ccs_database_categories' => 'category_id' )
					);
					break;
				case 'ccs_database_comments':
					return array(
						'tables'	=> array( 'ccs_database_comments' => 'comment_id' )
					);
					break;
				case 'database_entries':
					return array(
						'tables'	=> array( 'database_entries' => 'primary_id_field' )
					);
					break;
				case 'ccs_articles':
					return array(
						'tables'	=> array( 'ccs_custom_database_1' => 'primary_id_field' )
					);
					break;
				case 'ccs_databases':
					return array(
						'tables'	=> array( 'ccs_databases' => 'database_id' ),
						'where'		=> array( 'ccs_databases' => "database_id = 1" ) // It's probably not a good idea to delete the Articles database.
					);
					break;
				case 'ccs_folders':
					return array(
						'tables'	=> array( 'ccs_folders' => 'link_id' )
					);
					break;
				case 'ccs_page_templates':
					return array(
						'tables'	=> array( 'ccs_page_templates' => 'template_id' )
					);
					break;
				case 'ccs_pages':
					return array(
						'tables'	=> array( 'ccs_pages' => 'page_id' )
					);
					break;
				...
				// Why are there two of these? Audit and remove one.
				case 'attachments':
					return array(
						'tables'	=> array( 'attachments' => 'attach_id' ),
						'where'		=> array( 'attachments' => "attach_rel_module = 'ccs'" )
		...
		public function convertBlock($id, $info)
		{
			...
			//check key.
			//-----------------------------------------
			$_key	= $this->DB->buildAndFetch( array( 'select' => 'block_id', 'from' => 'ccs_blocks', 'where' => "block_key='{$info['block_key']}'" ) );

			if ( $_key['block_id'] )
			{
				$key = $info['block_key'].time();
				$this->logError($info['id'], "Block key {$info['block_key']} was already in use. Block was created with key {$key}");
				$info['block_key'] = $key;
			}
			//-----------------------------------------
			// Insert
		...
		public function convertTemplate($id, $info)
		{
			...
			//check key.
			//-----------------------------------------
			$_key	= $this->DB->buildAndFetch( array( 'select' => 'template_id', 'from' => 'ccs_page_templates', 'where' => "template_key='{$info['template_key']}'" ) );

			if ( $_key['template_id'] )
			{
				$key = $info['template_key'].time();
				$this->logError($info['id'], "Template key {$info['template_key']} was already in use. Template was created with key {$key}");
				$info['template_key'] = $key;
			}
			//-----------------------------------------
			// Insert
			//-----------------------------------------
		...
		public function convertFolder($path, $last_modified, $count)
		{
			...
			//check key.
			//-----------------------------------------
			$_key	= $this->DB->buildAndFetch( array( 'select' => 'folder_path', 'from' => 'ccs_folders', 'where' => "folder_path='{$path}'" ) );

			if ( $_key['folder_path'] )
			{
				$key = $path.time();
				$this->logError($path, "Folder {$path} was already in use. Folder was created with path {$key}");
				$path = $key;
			}
			//-----------------------------------------
			// Insert
		...
		public function convertPage($id, $info)
		{
			...
			if (!$info['page_seo_name'])
			{
				$info['page_seo_name'] = IPSText::makeSeoTitle($info['page_name']).'.html';
			}
			$info['page_folder'] = isset($info['page_folder'])?$info['page_folder']:'';
			...
			//check key.
			//-----------------------------------------
			$_key	= $this->DB->buildAndFetch( array( 'select' => 'page_id', 'from' => 'ccs_pages', 'where' => "page_folder='{$info['page_folder']}' AND page_seo_name='{$info['page_seo_name']}'" ) );

			if ( $_key['page_id'] )
			{
				$key = time().$info['page_seo_name'];
				$this->logError($id, "Page FURL {$info['page_folder']}{$info['page_seo_name']} was already in use. Page was created with FURL {$info['page_folder']}{$key}");
				$info['page_seo_name'] = $key;
			}
			//-----------------------------------------
			// Insert
			//-----------------------------------------
		...
		public function convertDatabaseCategory($id, $info)
		{
			...
			if (!$info['category_furl_name'])
			{
				$info['category_furl_name'] = IPSText::makeSeoTitle($info['category_name']);
			}
			...
			{
				$parent = $this->getLink($info['category_parent_id'], 'ccs_database_categories', TRUE);
				if ($parent)
				{
			...
			//check key.
			//-----------------------------------------
			$_key	= $this->DB->buildAndFetch( array( 'select' => 'category_id', 'from' => 'ccs_database_categories',
				'where' => "category_furl_name='{$info['category_furl_name']}' AND category_database_id={$info['category_database_id']}" ) );

			if ( $_key['category_id'] )
			{
				$key = $info['category_furl_name'].time();
				$this->logError($id, "Category FURL {$info['page_seo_name']} was already in use. Category was created with FURL {$key}");
				$info['category_furl_name'] = $key;
			}
			//-----------------------------------------
			// Insert
			//-----------------------------------------
		...
		public function convertDatabaseComment($id, $info)
		{
			...
			$info['comment_record_id'] = $this->getLink($info['comment_record_id'], 'ccs_articles');
			
			if ( ! $info['comment_record_id'] )
			{
				$this->logError( $id, 'No Article Found' );
				return false;
			}
