...
class admin_convert_board_smf extends ipsCommand
{
	...
	*/
	...
	public function countRows($action)
	{
		switch ($action)
		{
			...
			case 'profile_friends':
				$friends = ipsRegistry::DB('hb')->buildAndFetch(array(
					'select' => 'SUM(CHAR_LENGTH(buddy_list) - CHAR_LENGTH(REPLACE(buddy_list, ",", "") + 1)) AS total',
					'from'	 => 'members',
					'where'	 => 'buddy_list != ""'
				));
				
				return ( $friends['total'] ) ? $friends['total'] : 0;
				break;
				
			case 'ignored_users':
				$ignored = ipsRegistry::DB('hb')->buildAndFetch(array(
					'select' => 'SUM(CHAR_LENGTH(pm_ignore_list) - CHAR_LENGTH(REPLACE(pm_ignore_list, ",", "") + 1)) AS total',
					'from'	 => 'members',
					'where'	 => 'pm_ignore_list != ""'
				));

				return ( $ignored['total'] ) ? $ignored['total'] : 0;
				break;
	...
	private function fixPostData($text)
	{
		...
		//Quotes
		$text = preg_replace("#\[quote author=(.+?) link=(.+?)=(\d+)\.msg(\d+)(.+?)date=(\d+)\](.+)\[\/quote\]#i", "[quote name=\"$1\" post=\"$4\" timestamp=\"$6\"]$7[/quote]", $text);
	...
	private function convert_posts()
	{
		...
		while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
		{
			...
			$this->lib->setLastKeyValue( $row['id_msg'] );
			...
			if($rget)
			{
				...
				while ($rgetc = ipsRegistry::DB('hb')->fetch( $res ))
				{
					$comments[] = array(
							'rid'			=> $rget['id_report'],
							'comment'		=> $rgetc['body'],
				...
				$res2 = ipsRegistry::DB('hb')->execute();
				while ($rget2 = ipsRegistry::DB('hb')->fetch( $res2 ))
				{
					$reports[] = array(
							'id'			=> $rget2['id_report'],
	...
	private function convert_attachments()
	{
		...
		$this->lib->getMoreInfo('attachments', $loop, array('attach_path' => array('type' => 'text', 'label' => 'The path to the folder where attachments are saved (no trailing slash - usually path_to_smf/attachments - if you have multiple attachments paths in SMF then these will automatically be detected and used instead):')), 'path');
		...
		// SMF supports multiple attachment paths. If we have a folder ID, we can auto-detect the path.
		$attach = ipsRegistry::DB('hb')->buildAndFetch( array( 'select' => 'value', 'from' => 'settings', 'where' => "variable = 'attachmentUploadDir'" ) );
		$paths = unserialize( $attach['value'] );
		...
		while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
		{
			...
			// Are we missing the filext? Try to get it from the file itself.
			if ( ! $row['fileext'] )
			{
			...
			$save = array(
				...
				'attach_parent_id'		=> $post['id_topic']
				);
	...
	private function convert_polls()
	{
		...
		while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
		{
			...
			//-----------------------------------------
			// Check to make sure its corresponding
			// topic wasn't deleted.
			// thanks - http://community.invisionpower.com/topic/375423-error-when-converting-from-smf-poll-conversion-fails-if-corresponding-topic-has-been-deleted/?p=2352789
			//-----------------------------------------
			
			if ( !isset($topic['id_first_msg']) )
			{
				$this->lib->logError ( $topic['id_topic'], 'Poll topic has been deleted so the poll has not been converted.' );
				continue;
			}
	...
	private function convert_warn_logs()
	{
		...
		while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
		{
			...
			$save = array(
				'wl_member'			=> $row['id_recipient'],
				'wl_note_member'	=> $this->fixPostData( $row['body'] ),
				'wl_date'			=> $row['log_time'],
				'wl_points'			=> $row['counter'],
				'wl_moderator'		=> $row['id_member'],
			);
