	...
	class admin_convert_board_mybb extends ipsCommand
	{
	    ...
	    */
		...
		private function fixPostData($post)
		{
			...
			// And odd align tags
			$post = preg_replace( "#\[align=(.+?)\](.+?)\[/align\]#ism", "[$1]$2[/$1]", $post );
		...
		private function convert_members()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				if ($row['avatartype'] == 'upload')
				{
					...
					$profile['photo_location'] = preg_replace ( "/\?dateline=([0-9]+)/si", '', $profile['photo_location'] ); // MyBB drives me insane sometimes.
...
if ( !$topic ) { continue; }
		...
		private function convert_warn_logs()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$save = array(
					'wl_member'			=> $row['uid'],
					'wl_note_member'	=> $row['title'],
					'wl_date'			=> $row['dateline'],
					'wl_points'			=> 1,
					'wl_moderator'		=> $row['issuedby']
					);
				...
				if ($row['expired'])
				{
					$revoke = array(
						'wl_member'			=> $row['uid'],
						'wl_note_member'	=> $row['revokereason'],
						'wl_date'			=> $row['daterevoked'],
						'wl_points'			=> -1,
						'wl_moderator'		=> $row['revokedby']
						);
