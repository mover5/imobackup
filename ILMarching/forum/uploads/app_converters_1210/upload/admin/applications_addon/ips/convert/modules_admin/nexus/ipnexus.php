 ...
 * IP.Nexus Merge Tool
 * Last Update: $Date: 2013-05-02 18:23:18 -0400 (Thu, 02 May 2013) $
	...
	private function convert_nexus_packages ( )
	{
		...
		while ( $row = ipsRegistry::DB ( 'hb' )->fetch ( $this->lib->queryRes ) )
		{
			...
			$save = array (
				...
				'p_tax'					=> $row['p_tax'],
				'p_renew_options'		=> $row['p_renew_options'],
				...
				'p_reviewable'			=> $row['p_reviewable'],
				'p_review_moderate'		=> $row['p_review_moderate'],
				'p_image'				=> $row['p_image'],
				'p_methods'				=> $row['p_methods'],
				'p_group_renewals'		=> $row['p_group_renewals'],
				'p_rebuild_thumb'		=> $row['p_rebuild_thumb'],
			);
	...
	private function convert_nexus_invoices ( )
	{
		...
		while ( $row = ipsRegistry::DB ( 'hb' )->fetch ( $this->lib->queryRes ) )
		{
			...
			$save = array (
				...
				'i_paid'		=> $row['i_paid'],
				'i_status_extra'=> $row['i_status_extra'],
				...
				'i_po'			=> $row['i_po'],
				'i_notes'		=> $row['i_notes'],
				'i_shipaddress'	=> $row['i_shipaddress'],
			);
	...
	private function convert_nexus_support_departments ( )
	{
		...
		while ( $row = ipsRegistry::DB ( 'hb' )->fetch ( $this->lib->queryRes ) )
		{
			...
			$save		= array (
				...
				'dpt_packages'			=> $row['dpt_packages'],
				'dpt_position'			=> $row['dpt_position'],
				'dpt_email'				=> $row['dpt_email'],
	...
	private function convert_nexus_support_requests ( )
	{
		...
		while ( $row = ipsRegistry::DB ( 'hb' )->fetch ( $this->lib->queryRes ) )
		{
			$save = array (
				...
				'r_ar_notify'			=> $row['r_ar_notify'],
				'r_cfields'				=> $row['r_cfields'],
			);
	...
	private function convert_nexus_support_replies ( )
	{
		...
		while ( $row = ipsRegistry::DB ( 'hb' )->fetch ( $this->lib->queryRes ) )
		{
			$save = array (
				...
				'reply_cc'			=> $row['reply_cc'],
				'reply_raw'			=> $row['reply_raw'],
				'reply_textformat'	=> $row['reply_textformat'],
				//'reply_ip_address'	=> $row['reply_ip_address'],
			);
