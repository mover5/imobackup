 ...
 * Photopost
 * Last Update: $Date: 2013-05-21 20:46:41 -0400 (Tue, 21 May 2013) $
	...
	class admin_convert_gallery_photoplog extends ipsCommand
	{
	    ...
	    */
	    public function doExecute( ipsRegistry $registry )
		{
			...
			$forAll = array( 'gallery_categories'	=> array(),
	    ...
	    */
		...
		private function convert_gallery_categories()
		{
			...
			{
			/*	$save = array(
					'album_category_id'		=> $us['container_cat'],
				...
				$save = array(
					...
					'category_type'			=> 1, // Enforce Images Only type.
		...
		private function convert_gallery_images()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				if ( ! $set && $row['catid'] && $row['userid'] )
				{
					...
					$cat = $this->DB->buildAndFetch(
						array(
							...
							'where'		=> 'a.album_category_id=' . $this->lib->getLink( $row['catid'], 'gallery_categories' ) . ' AND a.album_owner_id=' . $this->lib->getLink( $row['userid'], 'members', false, true ) . " AND cl.foreign_id='s" . $row['setid'] . "'"
						)
					);
				...
				$save = array(
					...
					'image_album_id'		=> 's' . $row['setid'],
					'image_caption'			=> $row['title'],
					'image_description'		=> $row['description'],
					'image_directory'		=> $row['userid'],
