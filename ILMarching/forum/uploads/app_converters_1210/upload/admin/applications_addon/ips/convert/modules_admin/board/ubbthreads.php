 ...
 * UBB.Threads
 * Last Update: $Date: 2013-07-10 15:28:05 -0400 (Wed, 10 Jul 2013) $
	...
	class admin_convert_board_ubbthreads extends ipsCommand
	{
	    ...
	    */
		...
		private function fixPostData($post)
		{
			...
			// Update 'graemlins' (emoticons) URLs.
			$post = str_replace( '<<GRAEMLIN_URL>>/', $this->settings['board_url'] . '/public/style_emoticons/default/', $post );
			$post = str_replace( '&lt;&lt;GRAEMLIN_URL&gt;&gt;/', $this->settings['board_url'] . '/public/style_emoticons/default/', $post );

			//Quotes
			$post = preg_replace("#<div class=\"ubbcode-block\"><div class=\"ubbcode-header\">Quote:</div><div class=\"ubbcode-body\">Originally posted by (.+?):(.+)<\/div><\/div>#i", "[quote name=\"$1\"]$2[/quote]", $post);
			$post = preg_replace("#<div class=\"ubbcode-block\"><div class=\"ubbcode-header\">Originally Posted By: (.+?)</div><div class=\"ubbcode-body\">(.+)<\/div><\/div>#i", "[quote name=\"$1\"]$2[/quote]", $post);
		...
		private function convert_attachments()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$save = array(
					...
					'attach_parent_id'		=> $topic['TOPIC_ID'],
					);
