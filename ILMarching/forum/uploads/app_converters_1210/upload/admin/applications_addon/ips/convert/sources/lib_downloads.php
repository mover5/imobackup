...
class lib_downloads extends lib_master {
	...
	public function menuRow($action = '', $return = false) {
		switch ($action) {
			...
			case 'downloads_ccontent':
				$count = $this->DB->buildAndFetch(array('select' => 'COUNT(*) as count', 'from' => 'downloads_ccontent'));
				$return = array(
					'name' => 'Field Content',
					'rows' => $count['count'],
					'cycle' => 250,
			...
			case 'downloads_files_records':
				$count = $this->DB->buildAndFetch(array('select' => 'COUNT(*) as count', 'from' => 'downloads_files_records'));
				$return = array(
			...
			case 'downloads_filebackup':
				$count = $this->DB->buildAndFetch(array('select' => 'COUNT(*) as count', 'from' => 'downloads_filebackup'));
				$return = array(
					'name' => 'Revisions',
					'rows' => $count['count'],
					'cycle' => 250,
	...
	public function truncate($action) {
		switch ($action) {
			case 'downloads_categories':
				return array(
					'tables' => array('downloads_categories' => 'cid'),
				);
				break;

			case 'downloads_mimemask':
				return array(
					'tables' => array('downloads_mimemask' => 'mime_maskid'),
				);
				break;

			case 'downloads_mime':
				return array(
					'tables' => array('downloads_mime' => 'mime_id'),
				);
				break;

			case 'downloads_files':
				return array(
					'tables' => array('downloads_files' => 'file_id'),
				);
				break;

			case 'downloads_cfields':
				return array(
					'tables' => array('downloads_cfields' => 'cf_id'),
				);
				break;

			case 'downloads_comments':
				return array(
					'tables' => array('downloads_comments' => 'comment_id'),
				);
				break;

			case 'downloads_downloads':
				return array(
					'tables' => array('downloads_downloads' => 'did'),
				);
				break;

			case 'downloads_filebackup':
				return array(
					'tables' => array('downloads_filebackup' => 'b_id'),
				);
				break;

			case 'downloads_ccontent':
				return array(
					'tables' => array('downloads_ccontent' => 'file_id'),
				);
				break;

			case 'downloads_files_records':
				return array(
					'tables' => array('downloads_files_records' => 'record_id', 'downloads_filestorage' => 'storage_id'),
				);
				break;

			case 'downloads_mods':
				return array(
					'tables' => array('downloads_mods' => 'modid'),
				);
				break;
			default:
	...
	public function convertFile($id, $info, $cfields) {
		...
		$info['file_submitter'] = ($this->getLink($info['file_submitter'], 'members', true, true) ) ? $this->getLink($info['file_submitter'], 'members', true, true) : 0;
		$info['file_approver'] = ( $this->getLink($info['file_approver'], 'members', true, true) ) ? $this->getLink($info['file_approver'], 'members', true, true) : 0;
		$info['file_topicid'] = ( $this->getLink( $info['file_topicid'], 'topics', true, true ) ) ? $this->getLink( $info['file_topicid'], 'topics', false, true ) : 0;
	...
	public function convertFileRecord($id, $info, $options) {
		...
		$info['record_file_id'] = $this->getLink($info['record_file_id'], 'downloads_files', TRUE);
		if (!$info['record_file_id']) {
			$this->logError($id, 'Orphaned File Record');
			return false;
		}
		//-----------------------------------------
		// Move file
		...
		if ($info['record_storagetype'] == 'disk') {
			...
			if (!$info['storage_file'] and !$info['storage_ss'] and !$info['storage_thumb']) {
				$this->logError($id, 'No file data provided');
				return false;
			}
			$storage = array('storage_file' => $info['storage_file'], 'storage_ss' => $info['storage_ss']);
			$this->DB->insert('downloads_filestorage', $storage);
			unset($storage);
		}

		unset($info['record_id'], $info['storage_file'], $info['storage_ss'], $info['storage_thumb'], $info['storage_id']);
	...
	public function convertRevision($id, $info) {
		...
		$info['b_fileid'] = $this->getLink($info['b_fileid'], 'downloads_files', TRUE);
		if (!$info['b_fileid']) {
			$this->logError($id, 'Orphaned Revision');
			return false;
		}
	...
	public function convertFave($id, $info) {
		...
		// Make sure we have everything we need
		//-----------------------------------------
		$info['like_rel_id'] = $this->getLink($info['like_rel_id'], 'downloads_files', TRUE);
		$info['like_member_id'] = $this->getLink($info['like_member_id'], 'members', TRUE);


		$this->convertFollow(array(
			'like_app' => 'downloads',
			'like_area' => 'files',
			'like_rel_id' => $info['like_rel_id'],
			'like_member_id' => $info['like_member_id'],
