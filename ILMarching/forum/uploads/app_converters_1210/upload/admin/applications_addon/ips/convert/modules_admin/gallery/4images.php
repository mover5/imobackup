 ...
 * 4images
 * Last Update: $Date: 2013-08-01 10:51:53 -0400 (Thu, 01 Aug 2013) $
	...
	class admin_convert_gallery_4images extends ipsCommand
	{
	    ...
	    */
		...
		private function convert_gallery_albums()
		{
			...
			$this->lib->getMoreInfo('container_cat', $loop, array('orphans' => array('type' => 'dropdown', 'label' => 'To which category do you wish to put albums with no category?', 'options' => $cats)));
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$this->lib->convertAlbum ( $row['cat_id'], $save );
			}
