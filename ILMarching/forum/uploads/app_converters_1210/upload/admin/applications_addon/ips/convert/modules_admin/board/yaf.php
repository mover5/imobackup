 ...
 * Last Update: $Date: 2013-07-10 15:29:37 -0400 (Wed, 10 Jul 2013) $
	...
	*/
	...
	private function convert_members()
	{
		...
		while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
		{
			...
			$info = array(
				...
				'username'			=> $row['Name'],
				'email'				=> $row['Email'],
				'md5pass'			=> IPSText::mbstrtolower($row['Password']),
	...
	private function convert_posts()
	{
		...
		while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
		{
			...
			$this->lib->setLastKeyValue( $row['MessageID'] );
		}
		$this->lib->next();
	}
