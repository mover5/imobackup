 ...
 */
	...
	if (!is_numeric(ipsRegistry::$request['cat']))
	{
		// Boink to index
	...
	$row = $DB->buildAndFetch( array( 'select' => 'ipb_id', 'from' => 'conv_link', 'where' => "foreign_id='".ipsRegistry::$request['cat']."' AND type='gallery_categories' AND app='".$app['app_id']."'" ) );
	$registry->getClass('output')->silentRedirect(IPB_URL.'/index.php?app=gallery&module=cats&do=sc&cat='.$row['ipb_id'], '', TRUE);
