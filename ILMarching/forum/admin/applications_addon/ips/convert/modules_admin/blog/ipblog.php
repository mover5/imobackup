	    ...
	    */
	    public function doExecute( ipsRegistry $registry )
		{
			...
			$this->actions = array(
				...
				'blog_themes'	=> array(),
				'blog_blogs'	=> array('members', 'blog_themes', 'forum_perms'),
				...
				'blog_ratings'	 => array('blog_blogs', 'blog_entries')
				);
	    ...
	    */
		...
		private function convert_blog_blogs()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				while ($block = ipsRegistry::DB('hb')->fetch( $res ))
				{
...
**/
		...
		private function convert_blog_polls()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$res = ipsRegistry::DB('hb')->execute();
				while ($voter = ipsRegistry::DB('hb')->fetch( $res ))
				{
		...
		private function convert_blog_comments()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$rc = ipsRegistry::DB('hb')->buildAndFetch( array( 'select' => 'com_id', 'from' => 'rc_classes', 'where' => "my_class='blog'" ) );
				$rs = array(	'select' 	=> '*',
		 ...
		 * @return void
		 **/
		private function convert_blog_ratings()
		{
			//---------------------------
			// Set up
			//---------------------------
			...
			$loop = $this->lib->load('blog_ratings', $main);
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				$this->lib->convertRating($row['rating_id'], $row);
			}
			$this->lib->next();
		}
