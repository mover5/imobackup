 ...
 * phpBB
 * Last Update: $Date: 2013-09-12 17:44:25 -0400 (Thu, 12 Sep 2013) $
...
class admin_convert_board_e107 extends ipsCommand
{
	...
	*/
	...
	private function fixPostData($text)
	{
		...
		$text = str_replace ( '{e_FILE}', 'http://www.vr6oc.com/e107_files/', $text );
		
		// Now convert [link]
		//$text = preg_replace( '/\[link=(.+)\](.+)\[\/link\]/si', '[url=${1}]${2}[/url]', $text );
		
		// Now convert [file]
		//$text = preg_replace( '/\[file=(.+)\](.+)\[\/file\]/si', '[url=${1}]${2}[/url]', $text );
		
		// Let's not over complicate things here.
		$text = str_replace( array( '[file=', '[link=' ), '[url=', $text );
		$text = str_replace( array( '[/file]', '[/link]' ), '[/url]', $text );

		// ... and finally images.
		$text = preg_replace ( '/\[img:(width=\d+)?(&)?(height=\d+)?\](.+)\[\/img\]/iU', '[img]${4}[/img]', $text );
	...
	private function convert_members()
	{
		...
		//-----------------------------------------
		$pcpf = array( 'user_location'		=> 'Location',
						//'user_icq'		=> 'ICQ Number',
						//'user_aim'		=> 'AIM ID',
						//'user_yim'		=> 'Yahoo ID',
						//'user_msnm'		=> 'MSN ID',
		...
		$ask['pp_path']  	= array('type' => 'text', 'label' => 'Path to avatars uploads folder (no trailing slash, default /path_to_e107/e107_files/public/avatars): ');
		...
		while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
		{
			...
			$info = array( 'id'				=> $row['user_id'],
						   //'group'				=> ($row['user_admin']) == '1' ? ipsRegistry::$settings['admin_group'] : ipsRegistry::$settings['member_group'],
			...
			$members = array( 'ip_address'		=> $row['user_ip'],
				...
				'member_group_id'	=> ( $row['user_admin'] ? ipsRegistry::$settings['admin_group'] : ipsRegistry::$settings['member_group'] ),
			...
			if ( $row['user_image'] != '' && $row['user_image'] != NULL )
			{
				...
				else
				{
					...
					$profile['photo_location'] = str_replace( '-upload-', '', $row['user_image'] );
					$path = $us['pp_path'];
				}
			}
