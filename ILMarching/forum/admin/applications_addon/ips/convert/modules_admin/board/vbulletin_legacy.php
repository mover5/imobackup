 ...
 * vBulletin
 * Last Update: $Date: 2013-11-18 11:27:20 -0500 (Mon, 18 Nov 2013) $
	    ...
	    */
		...
		public function countRows($action)
		{
			switch ($action)
			{
				...
				case 'moderators':
					return $this->lib->countRows('moderator', "forumid != '-1'");
					break;
		...
		public function checkConf($action)
		{
			switch ($action)
			{
				...
				case 'forum_perms':
					return empty($this->request['app_merge']) ? true : false;
		...
		{
			$post = preg_replace("#\[sigpic\](.+?)\[\/sigpic\]#", "", $post);
			$post = str_replace("[/sigpic]", "", $post);
		...
		private function convert_groups()
		{
			...
			foreach( $loop as $row )
			{
				...
				$save = array(
					...
					'g_invite_friend'		=> 1,
					'g_edit_profile'		=> ( $row['canmodifyprofile'] ) ? 1 : 0,
		...
		private function convert_members()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				foreach ($pfields as $field)
				{
					...
					if ( $fieldsCheck[$field['ipb_id']]['pf_type'] == 'radio' )
					{
						$options = explode( "|", $fieldsCheck[$field['ipb_id']]['pf_content'] );
					
						foreach ( $options as $option )
						{
							$_option = explode( "=", $option );
								
							if ( $_option[1] == $row['field'.$field['foreign_id']] )
							{
								$custom['field_'.$field['ipb_id']] = $_option[0];
							}
						}
					}
				}
		...
		private function convert_forums()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				// vBulletin allows you to have root level, postable forums. We do not, so let's check for this.
				$isOrphan = false;
				if ( ! $this->lib->getLink( $row['parentid'], 'forums', true ) AND $row['parentid'] != -1 )
				{
					$isOrphan = true;
					
					// There is no parent... let's go ahead and create a pseudo forum if one does not exist.
					if ( ! $this->lib->getLink( 'orphan', 'forums', true ) )
					{
						$orphan = array(
						...
						$this->lib->convertForum( 'orphan', $orphan );
					}
				}
				...
				$save = array(
					...
					'parent_id'		  	=> ( $isOrphan ? 'orphan' : $row['parentid'] ),
		...
		private function convert_topics()
		{
			...
			$main = array(	'select' 	=> 't.*',
							...
							'add_join'		=> array(
								array(
									'select'	=> 'p.userid as last_poster_id',
									'from'	=> array( 'post' => 'p' ),
									'where'	=> 't.lastpostid=p.postid',
									'type'		=> 'left'
								)
							),
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				if ( $row['taglist'] )
				{
					$vbtags	= explode(',', $row['taglist']);
				}
				
				if( !empty( $vbtags ) )
				{
					foreach( $vbtags as $tag )
					{
						$tags[] = trim( $tag );
					}
						
					$save	=	array(
							'tag_meta_id'			=>	$row['threadid'], // could also use $row['threadid']
		...
		private function convert_pms()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$this->lib->setLastKeyValue($row['pmtextid']);
			}
		...
		private function convert_attachments()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				$this->lib->setLastKeyValue( $row['attachmentid'] );
				...
				$save = array(
					...
					'attach_parent_id'		=> $topicid['threadid'],
					'attach_location'		=> '',
					);
		...
		private function convert_pfields()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$save = array(
					...
					'pf_show_on_reg'=> $reg,
					'pf_group_id'	=> ($usegroup) ? $row['profilefieldcategoryid'] : $us['pfield_group'],
		...
		private function convert_emoticons()
		{
			...
			// @todo If $this->request['empty'] is set, do not ask about duplicates.
			//-----------------------------------------
			

			$this->lib->getMoreInfo('emoticons', $loop,
													array(
														'emo_path' => array(
																			'type' => 'text',
																			'label' => 'The path to the folder where emoticons are saved (no trailing slash - usually path_to_vb/images/smilies):'
																	),
														'emo_opt'  => array(
																			'type' => 'dupes',
																			'label' => 'How do you want to handle duplicate emoticons?'
																	)
													), 'path'
									);
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				// While this does work, it seems to be very slow if the source url's are slow - need to investigate further, perhaps add a timeout.
				//-----------------------------------------
				
				if ( preg_match( "#^http(s)?://[a-z0-9-_.]+\.[a-z]{2,4}#i", $row['smiliepath'], $matches ) )
				{
					// for ternerys sake
					$isUrl = true;
					...
					$grab							= new $classToLoad();
					//$grab->timeout 					= 15;
					
					// grab it
					$remote			= $grab->getFileContents( $row['smiliepath'] );

					// let's grab the extension from the image
					$ext = explode('.', str_replace( $matches, '', $row['smiliepath'] ) );
					$extension = array_pop( $ext );
					
					// format some form of filename
					$row['remote_smiley'] =  IPSText::htmlspecialchars(IPSText::mbstrtolower(stripslashes(str_replace(' ', '', $row['title']))) . $row['smilieid'] . '.') . $extension;
					
					// store it, for now we'll store these separately
					@mkdir(DOC_IPS_ROOT_PATH . '/public/style_emoticons/converted/', 0777);
					$file = file_put_contents(DOC_IPS_ROOT_PATH . '/public/style_emoticons/converted/' . $row['remote_smiley'], $remote);
					
					// catch those errors
					if ( !$file )
					{
						$this->lib->logError( $row['smiliepath'], 'Emoticon could not be retrieved or saved from remote path. You need to download the emoticon and add it <a href="'.$this->settings['base_url'].'app=core&&module=posts&section=emoticons&do=overview" target="_blank">here</a>.' );
					}
				}
				
				$save = array(
							'typed'			=> $row['smilietext'],
							'image'			=> $isUrl == true ? $row['remote_smiley'] : preg_replace('#^(.+)?images/smilies/(.+?)$#', '$2', $row['smiliepath']),
							'clickable'		=> 0,
							'emo_set'		=> $isUrl == true ? 'converted' : 'default',
							'emo_position'	=> $row['displayorder'],
						);

				unset($row['remote_smiley']);
		...
		private function convert_warn_logs()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$save = array(
					'wl_member'			=> $row['userid'],
					'wl_note_member'	=> $this->fixPostData($row['note']),
					'wl_date'			=> $row['dateline'],
					'wl_moderator'		=> $row['whoadded'],
