 ...
 */
	...
	class admin_convert_board_phpbb extends ipsCommand
	{
	    ...
	    */
		...
		{
			$text = nl2br($text);
			$text = html_entity_decode($text,ENT_COMPAT | ENT_HTML401, IPS_DOC_CHAR_SET);
...
[quote', $text);
			...
			// And URLs
			$text = preg_replace("#<a class=\"postlink\" href=\"([^\]]+?)\"([^>]+?)?>([^\]]+?)</a>#i", "[url=$1]$3[/url]", $text);
		...
		private function convert_attachments()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				// Check for attachments without an associated parent.
				if( $row['in_message'] )
				{
					if ( ! $parentId = $this->lib->getLink( $row['post_msg_id'], 'pms', true ) )
					{
						$this->lib->logError( $row['attach_id'], 'No Parent Conversation Found.' );
						continue;
					}
				}
				else
				{
					if ( ! $parentId = $this->lib->getLink( $row['topic_id'], 'topics', true ) )
					{
						$this->lib->logError( $row['attach_id'], 'No Parent Topic Found.' );
						continue;
					}
				}
				...
				$save = array(
					...
					'attach_parent_id'		=> $row['in_message'] ? $row['post_msg_id'] :  $row['topic_id'],
				);
		...
		private function convert_warn_logs()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$save = array(
					'wl_member'			=> $row['reportee_id'],
					'wl_note_member'	=> $this->fixPostData( $log[0] ),
					'wl_date'			=> $row['warning_time'],
					'wl_moderator'		=> $row['user_id'],
				);
