 ...
 * MyBB
 * Last Update: $Date: 2013-12-23 09:52:21 -0500 (Mon, 23 Dec 2013) $
	...
	*/
	...
	public function countRows($action)
	{
		switch ($action)
		{
			case 'forum_perms':
			case 'groups':
				return $this->lib->countRows('common_usergroup');
				break;
				
			case 'members':
				return  $this->lib->countRows('common_member');
				break;
				
			case 'forums':
				return $this->lib->countRows('forum_forum');
				break;
				
			case 'topics':
				return  $this->lib->countRows('forum_thread');
				break;
				
			case 'posts':
				return  $this->lib->countRows('forum_post');
				break;
			...
			case 'polls':
				return $this->lib->countRows('forum_poll');
				break;
				
			case 'attachments':
				return $this->lib->countRows('forum_attachment');
				break;
	...
	private function convert_members()
	{
		...
		$main = array(
			'select'	=> 'm.*',
			'from'		=> array( 'common_member' => 'm' ),
			'add_join'	=> array(
				array(
					'select'	=> 'p.*',
					'from'		=> array( 'common_member_profile' => 'p' ),
					'where'		=> 'm.uid = p.uid',
					'type'		=> 'left'
				),
				array(
					'select'	=> 'f.*',
					'from'		=> array( 'common_member_field_forum' => 'f' ),
					'where'		=> 'm.uid = f.uid',
					'type'		=> 'left',
				),
			),
...
//md5(md5($pass).$salt)
			...
			$members = array( 'ip_address'			  => '127.0.0.1',
							  ...
							  'email_pm'			  => $row['newprompt'],
							  'view_sigs'			  => 1,
							  'view_avs'			  => 1,
							  'msg_show_notification' => $row['newprompt'],
							  'last_visit'			  => time(),
							  'bday_day'				=> $row['birthday'],
							  'bday_month'				=> $row['birthmonth'],
							  'bday_year'				=> $row['birthyear'],
							  'last_activity'		  => time(),
							  'dst_in_use'			  => 0,
							  'coppa_user'			  => 0,
			...
			// And go!
			//-----------------------------------------
			$this->lib->convertMember($info, $members, $profile, array(), null);
			$this->lib->setLastKeyValue( $row['uid'] );
		}
		$this->lib->next();
	}
	...
	private function convert_forums()
	{
		...
		while( $row = ipsRegistry::DB('hb')->fetch( $this->lib->queryRes ) )
		{
			...
			$this->lib->setLastKeyValue( $row['fid'] );
		}
		$this->lib->next();
	}
	...
	private function convert_topics()
	{
		...
		while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
		{
			...
			$this->lib->setLastKeyValue( $row['tid'] );
		}
		$this->lib->next();
	}
	...
	private function convert_attachments()
	{
		...
		while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
		{
			// What table is our attachment in?
			if ( ! ipsRegistry::DB('hb')->checkForTable( 'forum_attachment_' . $row['tableid'] ) )
			{
				continue;
			}
			$attachment = ipsRegistry::DB('hb')->buildAndFetch( array( 'select' => '*', 'from' => 'forum_attachment_' . $row['tableid'], 'where' => "aid = {$row['aid']}" ) );
			// What's the extension?
			$ext = explode('.', $attachment['filename']);
			$extension = array_pop( $ext );

			// Sort out data
			$save = array( 'attach_rel_id'	   => $row['pid'],
						   'attach_parent_id'  => $row['tid'],
						   'attach_ext'		   => $extension,
						   'attach_file'	   => $attachment['filename'],
						   'attach_location'   => $attachment['attachment'],
						   'attach_is_image'   => $attachment['isimage'],
						   'attach_rel_module' => 'post',
						   'attach_member_id'  => $row['uid'],
						   'attach_hits'	   => $row['downloads'],
						   'attach_date'	   => $attachment['dateline'] );
			 // Send em on
			 $done = $this->lib->convertAttachment( $row['aid'], $save, $us['attach_path'] );
			 
			 // Fix Inline Attachments
			 if ( $done === true )
			 {
				 $aid = $this->lib->getLink( $row['aid'], 'attachments', true );
				 $pid = $this->lib->getLink( $row['pid'], 'posts', true );
				 
				 if ( $pid )
				 {
					 $post = $this->DB->buildAndFetch( array( 'select' => 'post', 'from' => 'posts', 'where' => "pid = {$pid}" ) );
					 
					 $update = str_replace( "[attach]{$row['aid']}[/attach]", "[attachment={$aid}:{$save['attach_location']}]", $post['post'] );
