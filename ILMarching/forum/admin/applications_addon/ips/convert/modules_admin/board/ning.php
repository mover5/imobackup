...
$custom = array(
	'directory' => 'Input directory of Ning json export. (Contains: ning-members.json, ning-discussions.json)',
	'app_name'	=> 'Unique Key for Conversion'
);
	...
	*/
	...
	private function convert_topics()
	{
		...
		foreach ( $discussionData as $row )
		{
			...
			$this->lib->convertTopic($topic_id[2], $save);
		}
		...
		foreach ( $discussionData as $row )
		{
			...
			foreach ( $row->comments as $post )
			{
				...
				$save = array( 'author_id'		=> $post->contributorName,
							   ...
							   'use_sig'     	=> 1,
							   'use_emo'     	=> 1,
							   'post_htmlstate'	=> 1, // Ning uses Raw HTML in their posts.
							   'ip_address' 	=> '127.0.0.1',
	...
	private function _parseMemberListFile()
	{
		...
		foreach( explode( '},{', $jsonData ) AS $k => $v )
		{
			$v = trim( $v, '{}' );
	
			$parsedData[$k] = json_decode( '{' . $v . '}', false );
		}
