 ...
 * Kunena 2.x Converter
 * Last Update: $Date: 2013-10-23 17:00:06 -0400 (Wed, 23 Oct 2013) $
...
class admin_convert_board_kunena extends ipsCommand
{
	...
	public function doExecute ( ipsRegistry $registry )
	{
		...
		$this->lib->sendHeader ( 'Kunena 2.x &rarr; IP.Board Converter' );
	...
	public function countRows ( $action )
	{
		switch ( $action )
		{
			...
			case 'forum_perms':
				...
				break;
			// gid in jos_kunena_users maps to jos_core_acl_aro_groups
			...
			case 'members':
				return $this->lib->countRows ( 'users' ); // are we using joomla_users or joomla_kunena_users - we can use either from what I can see.
				break;
			/**
			case 'moderators':
			...
			**/
			...
			case 'topics':
				return $this->lib->countRows ( $this->kunena_prefix . 'topics' );
				break;
	...
	/*
	 * Kunena 2.x doesn't have per-forum mods(?)
	 */
	/**
	...
	**/
	private function convert_topics()
	{
		...
		$main = array (
			'select'	=> '*',
			'from'		=> $this->kunena_prefix . 'topics',
			'order'		=> 'id ASC',
		);
		...
		while ( $row = ipsRegistry::DB ( 'hb' )->fetch ( $this->lib->queryRes ) )
		{
			$save = array (
				...
				'views'			=> $row['hits'],
				'starter_id'	=> $row['first_post_userid'],
				'start_date'	=> $row['first_post_time'],
				'forum_id'		=> $row['category_id'],
	...
	private function convert_attachments ( )
	{
		...
		while ( $row = ipsRegistry::DB ( 'hb' )->fetch ( $this->lib->queryRes ) )
		{
			...
			if ( $topic['parent'] )
			{
				$save['attach_parent_id'] = $topic['parent'];
			}
