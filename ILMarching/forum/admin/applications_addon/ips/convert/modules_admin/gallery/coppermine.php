 ...
 * Last Update: $Date: 2013-11-30 13:47:57 -0500 (Sat, 30 Nov 2013) $
		...
		private function convert_gallery_albums()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$save = array(
					...
					'album_type'		=> ($row['visibility']) ? 2 : 1,
					'album_category_id'	=> ($skip_cat_link) ? $us['container_cat'] : $row['category'],
		...
		private function convert_gallery_images()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				...
				$save = array(
					...
					'image_directory'				=>	$row['filepath'],
					'image_file_name'				=>	$row['filename'],
					'image_file_size'				=>	$row['filesize'],
					'image_file_type'				=>	$mime,
					'image_approved'				=>	($row['approved'] == 'YES') ? 1 : 0,
					'image_views'					=>	$row['hits'],
					'image_comments'				=>	$this->lib->countRows('comments', "pid={$row['pid']}"),
					'image_date'					=>	$row['ctime'],
					'image_ratings_total'			=>	$rating * $row['votes'],
		...
		private function convert_gallery_comments()
		{
			...
			while ( $row = ipsRegistry::DB('hb')->fetch($this->lib->queryRes) )
			{
				$save = array(
					...
					'comment_ip_address'		=> $row['msg_raw_up'],
