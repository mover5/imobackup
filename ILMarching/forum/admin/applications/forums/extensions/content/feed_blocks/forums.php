...
class feed_forums implements feedBlockInterface
{
	...
	public function executeFeed( $block, $previewMode=false )
	{
			...
			case 'topics':
				...
				{	
					...
					while( $r = $this->DB->fetch($outer) )
					{
						...
						if( $config['sortby'] == 'start_date' )
						{
							$memberIds[ intval($r['starter_id']) ]	= intval($r['starter_id']);
						}
						else
						{
		...
		foreach( $content as $_content )
		{
			...
			switch( $config['content'] )
			{
				case 'topics':
					if( $config['sortby'] == 'start_date' )
					{
						$memberRecord	= IPSMember::buildDisplayData( $memberData[ $_content['starter_id'] ] );
					}
					else
					{
		...
		if( $parseAttachments )
		{
			...
			$_return	= $_classAttach->renderAttachments( $htmlToParse, $attach_pids, 'topic', false );
