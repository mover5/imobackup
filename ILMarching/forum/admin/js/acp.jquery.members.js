...
(function($){
	...
	memberlist = function($){
		...
		var init = function( url, defaultType ){
			...
			$("#member_search").keyup( focusSearch ).blur( blurSearch );			// Search box
		...
		fetchResults = function( options, force )
		{
			...
			else
			{
				// do stuff
				$.ajax( {
					url: request_url.replace(/&amp;/g, '&') + '&_nosave=1',
					type: 'get',
					data: o,
					success: function(data){
						if( data['error'] )
		...
		doAdvancedSearch = function(e){
			...
			// @link http://community.invisionpower.com/resources/bugs.html/_/ip-board/also-check-secondary-groups-checkbox-when-unchecked-doesnt-revert-the-inclusion-r42350
