 ...
 * Matt Mecham
 * Last Updated: $Date: 2013-05-16 21:35:15 -0400 (Thu, 16 May 2013) $
 * </pre>
 *
...
abstract class classes_tag_abstract
{
	...
	public function getFormattedError()
	{
		if( $this->errorMsg )
		{
			...
			{
				case 'no_good_tags':
					return sprintf( $this->lang->words['no_good_tags'], $this->_getMinLen(), $this->_getMaxLen() );
				break;
				case 'too_few_tags':
	...
	/**
	 * Reset prefix ID for meta_id by earliest prefix
	 * @param	int		Meta ID
	 * @param	boolean	Recache tags
	 */
	public function resetPrefix( $metaId, $recache=true )
	{
		$tags 		= $this->getRawTagsByMetaId( $metaId );
		$sortedMate = array();
		
		foreach( $tags as $id => $tag )
		{
			$sortedMate[ $tag['tag_added'] . '.' . $tag['tag_id'] ] = $id;
		}
		
		ksort( $sortedMate );
		
		$tagId = array_shift( $sortedMate );
		
		if ( $tagId )
		{
			$this->DB->update( 'core_tags', array( 'tag_prefix' => 0 ), 'tag_meta_app=\'' . $this->getApp() . '\' AND tag_meta_area=\'' . $this->getArea() . '\' AND tag_meta_id=' . intval( $metaId ) );
			$this->DB->update( 'core_tags', array( 'tag_prefix' => 1 ), 'tag_meta_app=\'' . $this->getApp() . '\' AND tag_meta_area=\'' . $this->getArea() . '\' AND tag_id=' . intval( $tagId ) );
		}
		
		if ( $recache )
		{
			$this->rebuildCache( array( 'meta_id' => $metaId ) );
		}
	}
	...
	public function moveTagsByMetaId( $oldMetaIds, $newMetaId )
	{
		...
		{
			$raw[ $id ] = $this->getRawTagsByMetaId( $id );
			$keys[]     = $this->_getKey( array( 'meta_id' => $id ) );
			$this->_deleteStoredPermissionDataByMetaId( $id );
			$this->_deleteCache( array( 'meta_id' => $id ) );
		}
		
		/* Figure out if we have a prefix or not */
		if ( count( $raw ) )
		{
			foreach( $raw as $_id => $array )
			{
				foreach( $raw[ $_id ] as $id => $tag )
				{
					if ( $tag['tag_prefix'] )
					{
						$hasPrefix = true;
	 ...
	 *  @return		void
	 */
	public function rebuildCache( $where )
	{
		$this->_deleteCache( $where );
		$this->_addCache( $where );
	}
