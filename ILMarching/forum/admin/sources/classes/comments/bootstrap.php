...
abstract class classes_comments_renderer
{
	...
	public function fetchFormatted( $parent, $filters=array() )
	{
		...
		/* Got a reply? */
		if ( !empty( $this->request['_rcid'] ) && is_array( $this->request['_rcid'] ) )
		{
			$preReply = '';
	...
	public function fetch( $parent, $filters=array() )
	{
		...
		/* Joins */
		$_post_joins = array( array( 'select'	=> 'm.member_id as my_member_id, m.*',
		...
		while( $row = $this->DB->fetch( $oq ) )
		{
			...
			if( !$row['member_id'] AND $row['my_member_id'] )
			{
				$row['member_id']	= $row['my_member_id'];
			}
			...
			{
				if( $_i['ignore_topics'] AND $_i['ignore_ignore_id'] == $row['comment_author_id'] )
				{
			...
			if ( $this->settings['reputation_enabled'] )
			{
				...
				{
					if ( $this->settings['reputation_show_content'] && $row['rep_points'] < $this->memberData['_members_cache']['rep_filter'] && $this->settings['reputation_point_types'] != 'like' )
					{
						$row['_repignored'] = true;
					}
				}
	...
	public function delete( $parentId, $commentId, $memberData )
	{
		...
		{
			/* Permission test */
			$can = $this->can('delete', array( 'comment_parent_id' => $parentId ) );
