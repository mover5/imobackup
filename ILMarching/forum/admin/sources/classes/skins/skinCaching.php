...
EOF;
	...
	public function removeDeadCSSCaches( $setID )
	{
		...
		foreach( $cssKeep as $_setID => $data )
		{
			...
			$_path = IPS_PUBLIC_PATH . 'style_css/css_' . $_setID;
	...
	public function rebuildMasterCSS( $setId=0 )
	{
		/* Set directory */
		$setId		  = is_numeric( $setId ) ? intval( $setId ) : trim( $setId );
		$cssDir   = IPS_PUBLIC_PATH . 'style_css/' . $this->remapData['css'][ $setId ];
		$messages = array();
		$css      = array();
	...
	public function fetchDirectoryCSS( $dir )
	{
		/* Set directory */
		$css	  = array();
		$cssDir   = IPS_PUBLIC_PATH . 'style_css/' . $dir;
		$messages = array();
	...
	public function writeMasterSkinCss( $id, $dir )
	{
		...
		$path    = IPS_PUBLIC_PATH . 'style_css/' . $dir;
		$msgs    = array();
...
EOF;
	...
	protected function _writeCSSFile( $setID, $css_group, $css_content )
	{
		...
		if ( ! SAFE_MODE_ON )
		{
			...
			if ( is_writeable( IPS_PUBLIC_PATH . 'style_css/' ) === TRUE )
			{
				$good_to_go = 1;

				if ( ! is_dir( IPS_PUBLIC_PATH . 'style_css/css_' . $setID ) )
				{
					if ( ! @ mkdir( IPS_PUBLIC_PATH . 'style_css/css_' . $setID, IPS_FOLDER_PERMISSION ) )
					{
					...
					{
						@file_put_contents( IPS_PUBLIC_PATH . 'style_css/css_' . $setID . '/index.html', '' );
						@chmod( IPS_PUBLIC_PATH . 'style_css/css_'.$setID, IPS_FOLDER_PERMISSION );
						$good_to_go = 1;
					}
				}
				else
				{
					if ( is_writable( IPS_PUBLIC_PATH . 'style_css/css_' . $setID ) !== TRUE )
					{
						$this->_addErrorMessage( sprintf( $this->lang->words['cssdirnotwritaaaable'], $setID ) );
						$good_to_go = 0;
					}
					else if ( is_file( IPS_PUBLIC_PATH . 'style_css/css_'.$setID.'/'.$css_group.'.css' ) )
					{
						if ( is_writable( IPS_PUBLIC_PATH . 'style_css/css_'.$setID.'/'.$css_group.'.css' ) !== TRUE )
						{
		...
		if ( $good_to_go )
		{
			if ( $FH = @fopen( IPS_PUBLIC_PATH . 'style_css/css_'.$setID.'/'.$css_group.'.css', 'w' ) )
			{
				fwrite( $FH, $css_content, strlen($css_content) );
				fclose( $FH );
				@chmod( IPS_PUBLIC_PATH . 'style_css/css_'.$setID.'/'.$css_group.'.css', IPS_FILE_PERMISSION );
