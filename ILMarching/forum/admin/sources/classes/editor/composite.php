...
class classes_editor_composite
{
	...
	*/
	...
	public function process( $content )
	{
		...
		if ( $this->getIsHtml() AND IN_ACP AND str_replace( '<!-- isHtml:1 -->', '', $cleanContent ) !== '' )
		{
			$cleanContent = '<!-- isHtml:1 -->' . $cleanContent;
		}
		else
		{
			$cleanContent = str_replace( '<!-- isHtml:1 -->', '', $cleanContent );
		}
	...
	{
		$content = preg_replace( '#<a\s+?([^>|data-ipb=\'nomediaparse\']*?)href=["\']([^"\']+?)?["\']([^>]*?)?>(.+?)</a>#is', "<a data-ipb='nomediaparse' $1href='$2'$3>$4</a>" , $content );
