...
class classes_text_parser
{
	...
	/**
	 * Do not process media tags while auto-linking.
	 * @var bool
	 */
	protected static $NoMediaAutoLinkify = false;
...
// 				{
				...
				// Bad word filter for $!^& becomes $!^&amp;.  Submitted in a post that is <p>$!^&amp;</p> and </ is not a shift from non-word to word character
					...
					if ( strstr( $r['type'], '$' ) )
					{
						...
						if ( DOC_IPS_CHAR_SET == 'UTF-8' && IPSText::isUTF8( $test ) )
						{
...
//				}
	...
	protected function _retrieveSmiley( $_emoCode, $_emoImage )
	{
		...
		/**
		 * <#EMO_DIR#> can end up not replaced at this point in specific situations.
		 * @link http://community.invisionpower.com/resources/bugs.html/_/ip-board/emoticons-bug-with-pm-archives-r42796
		 */
		if ( strstr( $this->settings['emoticons_url'], '<#EMO_DIR#>' ) AND $this->registry->output->skin['set_emo_dir'] )
		{
			$this->settings['emoticons_url'] = str_replace( '<#EMO_DIR#>', $this->registry->output->skin['set_emo_dir'], $this->settings['emoticons_url'] );
		}
