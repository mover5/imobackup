...
class class_text_parser_bbcode extends classes_text_parser
{
	...
	public function toHtml( $txt="" )
	{
		...
		// Don't auto-link media.
		parent::$NoMediaAutoLinkify = true;
		
		$txt = $this->_autoLinkUrls( $txt );
		
		// Reset
		parent::$NoMediaAutoLinkify = false;
	...
	protected function _autoLinkUrls_CallBack( $matches )
	{
		...
		/* Is this a media URL? */
		if( ! parent::$NoMediaAutoLinkify and $this->settings['bbcode_automatic_media'] and isset( $this->_bbcodes['media'] ) and ( $this->_bbcodes['media']['bbcode_sections'] == 'all' or in_array( parent::$Perms['parseArea'], explode( ',', $this->_bbcodes['media']['bbcode_sections'] ) ) ) )
		{
			$media		= $this->cache->getCache( 'mediatag' );
