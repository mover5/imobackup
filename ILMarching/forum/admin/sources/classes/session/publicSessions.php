...
class publicSessions extends ips_MemberRegistry
{
	...
	{	
		...
		// Bug 40926
		//IPSCookie::set( "member_id", self::$data_store['member_id'] );
		...
		if (self::$data_store['member_id'])
		{
			...
			foreach ( $this->caches['login_methods'] as $k => $data )
			{
				if ( $data['login_folder_name'] == 'ipsconnect' and $data['login_enabled'] )
				{
					...
					{
						IPSCookie::set( "ipsconnect_" . md5( $ipsConnectSettings['master_url'] ), 
								IPSCookie::get( "ipsconnect_" . md5( $ipsConnectSettings['master_url'] ), FALSE ), 
								( $this->settings['login_key_expire'] ? 0 : 1 ), $this->settings['login_key_expire'], FALSE, FALSE );
					}
				}
			}
			...
			if( IPSCookie::get( "ipsconnect_" . md5( $this->settings['board_url'] . '/interface/ipsconnect/ipsconnect.php' ), FALSE ) )
			{
				IPSCookie::set( "ipsconnect_" . md5( $this->settings['board_url'] . '/interface/ipsconnect/ipsconnect.php' ), 
						IPSCookie::get( "ipsconnect_" . md5( $this->settings['board_url'] . '/interface/ipsconnect/ipsconnect.php' ), FALSE ), 
						( $this->settings['login_key_expire'] ? 0 : 1 ), $this->settings['login_key_expire'], FALSE, FALSE );
			}
